...
def run_with_job_array(climate_suffix, gdirs=None, run_for_test=False,
    ...
    if output_dir is None:
        outpath = utils.mkdir(os.path.join(cluster_dir, 'cluster_output', 'Climate_2'), reset=False)
    else:
    ...
    prcp_fpath = os.path.join(data_dir, 'Climate2_99years', 'PrecipMean_'+climate_suffix+'.nc')
    temp_fpath = os.path.join(data_dir, 'Climate2_99years', 'T2mMean_'+climate_suffix+'.nc')
...
def plot_for_check_the_test_result():
    ...
    path = os.path.join(cluster_dir, 'cluster_output', 'Climate_2')
